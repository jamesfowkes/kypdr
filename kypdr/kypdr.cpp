...
static bool sb_single_press_mode = false;
...
static void keydown(uint8_t r, uint8_t c)
{
	char key = CHARACTERS[(r*6)+c];
	Keyboard.press(key);
}

static void keyup(uint8_t r, uint8_t c)
{
	char key = CHARACTERS[(r*6)+c];
...
static void s_debounce_task_fn(ADLTask& thisTask, void * pTaskData)
{
	...
	for (uint8_t c = 0; c<6;c++)
	{
		...
		if (!sb_single_press_mode)
		{
			if (s_debouncers[s_row][c].check_high_and_clear())
			{
				keyup(s_row, c);
			}
		}
...
void adl_custom_loop(DeviceBase * pdevices[], int ndevices, ParameterBase * pparams[], int nparams)
{
	...
	sb_single_press_mode = ((BooleanParam*)pparams[0])->get();
}
